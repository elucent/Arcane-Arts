...
public class crystalGrinderEntity extends TileEntity implements ITickable, tileEnergyContainer {
	public int ticker = 0;
			...
			if (getElementValue("air") > 8.0){
				if (getWorld().getBlockState(getPos().up()).getBlock() == aaItemManager.arcaneFocus){
					...
					if (afe.heldItem != null){
						ItemStack result = aaRecipeManager.getGrindingResult(afe.heldItem);
						if (result != null){
							ItemStack spawnItem = new ItemStack(result.getItem(),result.stackSize,result.getItemDamage());
							afe.heldItem.stackSize --;
							setElementValue("air",getElementValue("air")-8.0);
							if (afe.heldItem.stackSize == 0){
								afe.heldItem = null;
							}
							if (getWorld().isRemote == false){
								getWorld().spawnEntityInWorld(new EntityItem(getWorld(),getPos().getX()+0.5,getPos().getY()+2.0,getPos().getZ()+0.5,spawnItem));
							}
						}
					}
				}
