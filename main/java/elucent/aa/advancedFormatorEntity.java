...
public class advancedFormatorEntity extends TileEntity implements ITickable{
	int ticker = 0;
	...
	public void update() {
		...
		if (ticker >= 100){
			...
			if (recipe != null){
				...
				double fireReq = recipe.evFire.value;
				double earthReq = recipe.evEarth.value;
				double waterReq = recipe.evWater.value;
				double airReq = recipe.evAir.value;
				double lightReq = recipe.evLight.value;
				double voidReq = recipe.evVoid.value;
				double class1Req = 0;
				double class2Req = 0;
				if (recipe.itemClass1 != null){
					class1Req = recipe.itemClass1.value;
				}
				if (recipe.itemClass2 != null){
					class2Req = recipe.itemClass2.value;
				}
				int k = 0;
