...
public class waterInfuserEntity extends TileEntity implements ITickable {
	public int progress = 0;
	public ItemStack heldItem = null;
	int ticker = 0;
	Random random = new Random();
	public boolean filled = false;
	public waterInfuserEntity(){
		super();
	}
	...
	public void update() {
		...
		if (progress > 0){
			progress --;
			getWorld().spawnParticle(EnumParticleTypes.REDSTONE, getPos().getX()+0.25+0.5*random.nextFloat(), getPos().getY()+0.25+0.5*random.nextFloat(), getPos().getZ()+0.25+0.5*random.nextFloat(), -0.875, 0.0, 0.75, 0);
			if (progress == 0 && filled){
				for (int i = 0; i < 16; i ++){
					getWorld().spawnParticle(EnumParticleTypes.REDSTONE, getPos().getX()+random.nextFloat(), getPos().getY()+random.nextFloat(), getPos().getZ()+random.nextFloat(), -0.875, 0.0, 0.75, 0);
				}
		...
		if (player.getHeldItem() != null){
			...
			if (player.getHeldItem().getItem() == aaItemManager.itemAlchemyFlask){
				if (world.getBlockState(getPos()).getBlock().getMetaFromState(state) == 1){
					if (stack.getMetadata() == 0){
						if (stack.hasTagCompound()){
							...
							stack.getTagCompound().setString("elementName","water");
					...
					if (stack.getMetadata() == 1){
						if (stack.hasTagCompound()){
							if (stack.getTagCompound().hasKey("elementName") && stack.getTagCompound().hasKey("elementValue")){
								if (aaElementManager.getElementFromString(stack.getTagCompound().getString("elementName")).equals(aaElementManager.elementWater)){
				...
				if (stack.getItem() instanceof itemElementContainer){
					if (((itemElementContainer)stack.getItem()).isElementValid(stack, "water")){
						((itemElementContainer)(stack.getItem())).setElementValue(stack, "water", ((itemElementContainer)(stack.getItem())).getElementValue(stack,"water")+4.0);
						heldItem = stack;
						stack.stackSize = 0;
						progress = 20;
					}
					...
					for (int i = 0; i < recipes.size() && !doBreak; i ++){
						if (recipes.get(i).element == aaElementManager.elementWater){
