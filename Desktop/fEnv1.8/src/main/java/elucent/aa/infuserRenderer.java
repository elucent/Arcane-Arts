	...
	public void renderTileEntityAt(TileEntity te, double x, double y, double z, float partialTicks, int destroyStage) {
		...
		if (te instanceof waterInfuserEntity){
			...
			if (((waterInfuserEntity)te).filled){
				...
				worldRenderer.begin(7, DefaultVertexFormats.POSITION_TEX_NORMAL);
				addBlock(worldRenderer, 6.0D,9.0D,6.0D,10.0D,13.0D,10.0D,0.0D,0.0D,0.25,0.25);
				Tessellator.getInstance().draw();
			}
			worldRenderer.reset();
			GL11.glEnable(GL11.GL_BLEND);
			...
			GL11.glColor4f(1.0f, 1.0f, 1.0f, 0.75f);
			worldRenderer.begin(7, DefaultVertexFormats.POSITION_TEX_NORMAL);
			addBlock(worldRenderer, 5.0D,8.0D,5.0D,11.0D,14.0D,11.0D,0.0D,0.0D,0.375,0.375);
			Tessellator.getInstance().draw();
			GL11.glDisable(GL11.GL_BLEND);
			GL11.glEnable(GL11.GL_CULL_FACE);
			//GL11.glEnable(GL11.GL_LIGHTING);
